...
class TESSQLPTS(TESSTS):
    fnformat = 'hlsp_qlp_tess_ffi_s*-{}_*_llc.fits'
    ...
    def tic_from_name(cls, f: Path):
        return int(f.name.split('-')[1].split('_')[0])
    ...
    def can_read_input(source: Path) -> bool:
        ...
        """
        try:
            dfile = sorted(source.glob('hlsp_qlp_tess*_llc.fits'))[0] if source.is_dir() else source
            h = getheader(dfile)
            return 'MIT/QLP' in h['ORIGIN'] and 'TESS' in h['TELESCOP']
        except:
            return False
